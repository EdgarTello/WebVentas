...
namespace TwoTecnology.WebVentas.Formulario.GestorDocumental.LocalPerfil
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string LocalAreacodigo = ticket.UserData.Split('|')[10];
                ...
                using (Repositorio.Perfil obj = new TwoTecnology.Repositorio.Perfil())
                {
                    if (id == "0")
                    {
                        if (obj.ExisteByNombre(idlocal, nombre))
                            respuesta = "El area existe en el local seleccionado!";
                        else
                            obj.Agregar(idlocal, nombre );
                    }
                    else
            ...
            if (Request.QueryString["ope"] == "4")
            {
                ...
                {
                    respuesta = obj.LoadByPadre(idlocal).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "5")
            {
                ...
                using (Repositorio.Ubicacion obj = new TwoTecnology.Repositorio.Ubicacion())
                {
                    respuesta = obj.LoadByHijos(idubicacion).Serialize();
                }
            }
