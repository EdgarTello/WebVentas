    ...
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "1")
            {
                ...
                Metodos.ValidateAntiXsrfToken("EspacioList");

                using (Repositorio.GestorDocumento obj = new TwoTecnology.Repositorio.GestorDocumento())
                {
                    respuesta = obj.LoadSize(ruc, localcodigo).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "1_1")
            {
                ...
                using (Repositorio.GestorDocumento obj = new TwoTecnology.Repositorio.GestorDocumento())
                {
                    respuesta = obj.LoadSizeDetalle(ruc, localcodigo).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "10")
            {
                ...
                string idarchivador = Request.QueryString["archivador"];

                Metodos.ValidateAntiXsrfToken("HistorialList");

                using (Repositorio.GestorDocumento obj = new TwoTecnology.Repositorio.GestorDocumento())
                {
                    respuesta = obj.LoadHistorial(idlocal, idarchivador, usuario, evento, fechainicio, fechafin, ruc).Serialize();
                }
            }
