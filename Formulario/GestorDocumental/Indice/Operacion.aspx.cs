...
namespace TwoTecnology.WebVentas.Formulario.GestorDocumental.Indice
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "0")
            {
                ...
                using (Repositorio.GestorIndice obj = new TwoTecnology.Repositorio.GestorIndice())
                {
                    respuesta = obj.LoadByCodigo(id).Serialize();
                }
            }

            if (Request.QueryString["ope"] == "0_1")
            {
                string idarchivador = Request.QueryString["idarchivador"];

                using (Repositorio.GestorIndice obj = new TwoTecnology.Repositorio.GestorIndice())
                {
                    respuesta = obj.LoadIndices(idarchivador).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "1")
            {
                ...
                using (Repositorio.GestorIndice obj = new TwoTecnology.Repositorio.GestorIndice())
                {
                    respuesta = obj.Load(idarchivador).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "1_1")
            {
                ...
                if (idarchivador.IsNumeric())
                {

                    using (Repositorio.GestorIndice obj = new TwoTecnology.Repositorio.GestorIndice())
                    {
                        respuesta = obj.LoadByArchivador(idarchivador).Serialize();
                    }
                }
            }
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                {
                    if (id == "0")
                        if (obj.ExisteByNombre(ruc, idarchivador, nombre))
                            respuesta = "El índice existe!";
                        else
                            obj.Agregar(idarchivador, nombre, alias, tipo, datos,  ruc);
                    else
                        obj.Editar(id, nombre, alias, tipo, datos);
