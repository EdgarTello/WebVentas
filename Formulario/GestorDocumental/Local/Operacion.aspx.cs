...
namespace TwoTecnology.WebVentas.Formulario.GestorDocumental.Local
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "1_2")
            {
                ...
                Metodos.ValidateAntiXsrfToken("LocalList");

                if (!busqueda.IsInyection())
                {

                    using (Repositorio.Local obj = new TwoTecnology.Repositorio.Local())
                    {
                        respuesta = obj.Load(ruc, busqueda).Serialize();
                    }
                }
            }
            ...
            if (Request.QueryString["ope"] == "2_1")
            {
                ...
                string activo = Request.QueryString["activo"];
                string rutadocumento = Request.QueryString["rutadocumento"];
                string rutaprincipal = Request.QueryString["rutaprincipal"];

                Metodos.ValidateAntiXsrfToken("LocalNew");

                if (id.IsNumeric() && !nombre.IsInyection() &&
                    !departamento.IsInyection() && !provincia.IsInyection() && !distrito.IsInyection() &&
                    !direccion.IsInyection() && !ubigeo.IsInyection() &&
                    !telefono.IsInyection() && !latitud.IsInyection() && !longitud.IsInyection() &&
                    activo.IsNumeric() &&
                    !rutadocumento.IsInyection() && !rutaprincipal.IsInyection())
                {
                        ...
                        using (Repositorio.Local obj = new TwoTecnology.Repositorio.Local())
                        {
                            if (id == "0")
                            {
                                if (obj.ExisteByNombre(ruc, nombre))
                                    ...
                                    obj.Agregar_v3(ruc, nombre, departamento, provincia, distrito, direccion, ubigeo,
                                        telefono, latitud, longitud, rutadocumento, rutaprincipal, activo);
                            }
                            else
                                obj.Editar_v3(id, nombre, departamento, provincia, distrito, direccion, ubigeo,
                                    telefono, latitud, longitud, rutadocumento, rutaprincipal, activo);
                        }
                    }
                }
