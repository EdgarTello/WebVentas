...
namespace TwoTecnology.WebVentas.Formulario.GestorDocumental.Archivador
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string idperfil = ticket.UserData.Split('|')[30];
            string respuesta = "";
            ...
            if (Request.QueryString["ope"] == "1")
            {
                using (Repositorio.GestorArchivador obj = new TwoTecnology.Repositorio.GestorArchivador())
                {
                    respuesta = obj.Load(localcodigo, ruc).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "1_1")
            {
                ...
                using (Repositorio.GestorArchivador obj = new TwoTecnology.Repositorio.GestorArchivador())
                {
                    respuesta = obj.LoadByArea(localcodigo, idarea, idperfil, ruc).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                using (Repositorio.GestorArchivador obj = new TwoTecnology.Repositorio.GestorArchivador())
                {
                    ...
                    if (obj.ExisteByNombre(ruc, idarea, nombre))
                            respuesta = "El archivor existe!";
                    else
                        if (id == "0")
                            obj.Agregar(idarea, nombre,  ruc);
                        else
                            obj.Editar(id, idarea, nombre);
