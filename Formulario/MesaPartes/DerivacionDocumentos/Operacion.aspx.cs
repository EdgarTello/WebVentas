...
namespace TwoTecnology.WebVentas.Formulario.MesaPartes.DerivacionDocumentos
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "000" && usuario_acceso == "Derivador")
            {
                string id = Request.QueryString["id"];

                using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                    respuesta = obj.LoadByCodigo(ruc, id, "", "0").Serialize();
                
            }

            if (Request.QueryString["ope"] == "001" && usuario_acceso == "Derivador")
            {
                string id = Request.QueryString["id"];
            ...
            if (Request.QueryString["ope"] == "002" && (usuario_acceso == "Operador" || usuario_acceso == "Recepcion" || usuario_acceso == "Derivador" || usuario_acceso == "Operador"))
            {
            ...
            if (Request.QueryString["ope"] == "100" && usuario_acceso == "Derivador")
            {
                using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                {
                    ...
                    //respuesta = obj.LoadDerivador(ruc, fechainicio, fechafin ).Serialize();
            ...
            if (Request.QueryString["ope"] == "300" && usuario_acceso == "Derivador")
            {
                string id = Request.QueryString["id"];

                //if (etapa == "110" || etapa == "120")
                //{
                    using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                    {
                        obj.Eliminar(id, usuario_codigo);
                    }
                //}
                //else
                //{
                //    using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                //    {
                //        obj.Eliminar(id, usuario_nombre, "1");
                //    }
                //}
