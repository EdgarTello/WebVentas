...
namespace TwoTecnology.WebVentas.Formulario.MesaPartes.OperadorDocumentos
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "000" && (usuario_acceso == "Operador"|| usuario_acceso == "Gerencia"))
            {
                string id = Request.QueryString["id"];
                string etapa = Request.QueryString["etapa"] ?? "0";

                using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                    respuesta = obj.LoadByCodigo(ruc, id, "", etapa).Serialize();
                
            }

            if (Request.QueryString["ope"] == "001" && (usuario_acceso == "Operador" || usuario_acceso == "Gerencia"))
            {
                string id = Request.QueryString["id"];
            ...
            if (Request.QueryString["ope"] == "100" && (usuario_acceso == "Operador" || usuario_acceso == "Gerencia"))
            {
                using (TwoTecnology.Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento())
                {
                    ...
                    //respuesta = obj.LoadOperador(ruc, usuario_codigo, fechainicio, fecha ).Serialize();
            ...
            if (Request.QueryString["ope"] == "300" && (usuario_acceso == "Operador" || usuario_acceso == "Gerencia"))
            {
                string id = Request.QueryString["id"];
                using (Repositorio.MPDocumento obj = new TwoTecnology.Repositorio.MPDocumento()) 
                {
