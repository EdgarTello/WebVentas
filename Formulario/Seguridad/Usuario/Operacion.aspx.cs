    ...
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                string comision = Request.QueryString["comision"];
                string idrol = Request.QueryString["idrol"];
                ...
                string chkcookie = Request.QueryString["chkcookie"];
                string vcookie = string.Empty;

                Guid newGuid = Guid.NewGuid();
                vcookie = newGuid.ToString();
                

                Metodos.ValidateAntiXsrfToken("UsuarioNew");

                if (id.IsNumeric() && !nombre.IsInyection() &&
                    !apellidos.IsInyection() && !usuario.IsInyection() && !password.IsInyection() &&
                    !email.IsInyection() &&
                    idrol.IsNumeric() && idlocal.IsNumeric() && bloqueado.IsNumeric() &&
                    logueo.IsNumeric())
                {
                        ...
                        using (Repositorio.Empleado obj = new TwoTecnology.Repositorio.Empleado())
                        {
                            ...
                            if (id == "0")
                            {
                                ...
                                else
                                {
                                    ...
                                    obj.Agregarv10(ruc, nombre, apellidos, email, usuario, password, idrol, idlocal, bloqueado, logueo, comision);
                                }

                            }
                            else
                            {
                                obj.Editarv10(id, nombre, apellidos, email,usuario, password, idrol, idlocal, bloqueado, logueo, comision);

                                if (chkcookie == "1")
                                    obj.EditarCookie(id, vcookie);
                                else
                                {
                                    if (cookie == "")
                                        obj.EditarCookie(id, "");
