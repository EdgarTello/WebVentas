...
namespace TwoTecnology.WebVentas.Formulario.PuntoVenta.TipoPago
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string TipoPagocodigo = ticket.UserData.Split('|')[10];
            ...
            if (Request.QueryString["ope"] == "0")
            {
                using (Repositorio.TipoPago obj = new TwoTecnology.Repositorio.TipoPago())
                {
                    ...
                    respuesta = obj.LoadbyTipo(ruc, defecto, di).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "11")
            {
                using (Repositorio.TipoPago obj = new TwoTecnology.Repositorio.TipoPago())
                {
                    respuesta = obj.LoadAll(ruc).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                using (Repositorio.TipoPago obj = new TwoTecnology.Repositorio.TipoPago())
                {
                    obj.Agregar(ruc, nombre, orden, di, activo);
                }
