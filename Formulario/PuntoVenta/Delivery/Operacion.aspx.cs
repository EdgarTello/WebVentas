...
namespace TwoTecnology.WebVentas.Formulario.PuntoVenta.Delivery
{
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                if (!blnerror)
                {
                    ...
                    using (Repositorio.Cliente obj = new TwoTecnology.Repositorio.Cliente())
                    {
                        if (id == "0")
                        {
                            if (tipo_documento == "0" && (num_documento == "00000000" || num_documento == "-"))
                            {
                                respuesta = obj.Agregar(ruc, nombre, "M", "", tipo_documento, num_documento,
                                           departamento, provincia, distrito, direccion, referencia, ubigeo, telefonofijo,
                                           telefonocelular, email, "", "", contacto,"","", "0", "");
                            }
                            else { 
                            ...
                            respuesta=obj.Editar(id, nombre, "M", "", tipo_documento, num_documento,
                                departamento, provincia, distrito, direccion, referencia, ubigeo, telefonofijo,
                                telefonocelular, email, "", "", contacto, "", "", "0", "");
