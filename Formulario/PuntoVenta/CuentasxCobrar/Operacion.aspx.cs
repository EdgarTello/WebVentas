    ...
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "1")
            {
                ...
                if (usuario_acceso == "Contador" || usuario_acceso == "SuperAdmin")
                {   
                    idcaja = "0";

                    using (Repositorio.Pedido obj = new TwoTecnology.Repositorio.Pedido())
                    {
                        respuesta = obj.LoadByCajaCreditoFilter(
                            idlocal,
                            idcaja,
                            fechainicio,
                            fechafin,
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                string tipooperacion = Request.QueryString["tipooperacion"];

                using (Repositorio.Pedido obj = new TwoTecnology.Repositorio.Pedido())
                {
                    obj.ActualizarForma(id, idpedido, fecha, operacion, monto, tipooperacion);
                }
            }
            ...
            if (Request.QueryString["ope"] == "11")
            {
                ...
                using (Repositorio.Pedido obj = new TwoTecnology.Repositorio.Pedido())
                {
                    respuesta = obj.LoadPagos(id).Serialize();
