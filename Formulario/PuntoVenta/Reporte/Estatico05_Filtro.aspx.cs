...
{
    public partial class Estatico05_Filtro : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                cbocaja.Items.Clear();
                cbocategoriacliente.Items.Clear();
                cbocliente.Items.Clear();
                cbomesa.Items.Clear();
                cbocierre.Items.Clear();

                cbocaja.Items.Add(new ListItem { Value = "", Text = "[Todos]" });
                cbocategoriacliente.Items.Add(new ListItem { Value = "", Text = "[Todos]" });
                cbocliente.Items.Add(new ListItem { Value = "", Text = "[Todos]" });
        ...
        private void LoadCombos() {
            ...
            using (Repositorio.ClienteCategoria obj = new TwoTecnology.Repositorio.ClienteCategoria())
            {
                var listacategoriacliente = obj.LoadActivo(empresaruc);

                cbocategoriacliente.Items.Add(new ListItem { Value = "", Text = "[Todos]" });

                foreach (var item in listacategoriacliente)
                {
                    cbocategoriacliente.Items.Add(new ListItem { Value = item.id, Text = item.nombre });
                }
            }

        }

        protected void cbocategoriacliente_SelectedIndexChanged(object sender, EventArgs e)
        {
            string empresaruc = hruc.Value;
            cbocliente.Items.Clear();
            ...
            {
                var listacliente = obj.LoadByCategoria(empresaruc, cbocategoriacliente.SelectedValue);

                cbocliente.Items.Add(new ListItem { Value = "", Text = "[Todos]" });

                foreach (var item in listacliente)
                {
                    cbocliente.Items.Add(new ListItem { Value = item.id, Text = item.nombre });
                }
            }
        }
    }
