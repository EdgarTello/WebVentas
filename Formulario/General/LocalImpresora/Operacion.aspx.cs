...
namespace TwoTecnology.WebVentas.Formulario.General.LocalImpresora
{
    public partial class Operacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ope"] == "10")
            {
                ...
                {
                    string idlocal = Request.QueryString["idlocal"];
                    respuesta = obj.LoadPrecuentaByLocal(idlocal).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "11")
            {
                ...
                {
                    string idlocal = Request.QueryString["idlocal"];
                    respuesta = obj.LoadDocumentoByLocal(idlocal).Serialize();
                }
            }
            ...
            if (Request.QueryString["ope"] == "2")
            {
                ...
                string precuenta = Request.QueryString["precuenta"];
                string documento = Request.QueryString["documento"];
                string sonido = Request.QueryString["sonido"];
                ...
                using (Repositorio.Impresora obj = new TwoTecnology.Repositorio.Impresora())
                {
                    if (id == "0")
                    {
                        if (obj.ExisteByNombre(idlocal, nombre))
                            respuesta = "La impresora existe en el local seleccionado!";
                        else
                            obj.Agregar(idlocal, nombre, ip, precuenta, documento, sonido);
                    }
                    else
                        obj.Editar(id, nombre, ip, precuenta, documento, sonido);
                }
