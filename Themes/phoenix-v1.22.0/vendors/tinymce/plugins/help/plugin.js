...
(function () {
    ...
    const shortcuts = [
      ...
      {
        shortcuts: ['Meta + F'],
        action: 'd (if searchreplace plugin activated)'
      },
      {
    ...
    const tab$1 = editor => {
      ...
      const getPluginData = (editor, key) => d(urls, x => {
        return x.key === key;
      }).fold(() => {
      ...
      const getPluginKeys = editor => {
        ...
        return isUndeed(forcedPlugins) ? keys$1 : filter(keys$1, k => !contains(forcedPlugins, k));
      };
