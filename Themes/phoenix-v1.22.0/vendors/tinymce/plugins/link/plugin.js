...
(function () {
    ...
    const dTextByValue = (value, catalog) => dMap(catalog, item => {
      if (isListGroup(item)) {
        return dTextByValue(value, item.items);
      } else {
    ...
    const getDelta = (persistentText, fieldName, catalog, data) => {
      ...
      return value !== undeed ? dTextByValue(value, catalog).map(i => ({
        url: {
          value: i.value,
          meta: {
    ...
    const dCatalog = (catalogs, fieldName) => {
      if (fieldName === 'link') {
    ...
    const init = (initialData, linkCatalog) => {
      ...
      const onCatalogChange = (data, change) => {
        const catalog = dCatalog(linkCatalog, change).getOr([]);
    ...
    const preprocess = (editor, data) => dMap([
      tryEmailTransform,
    ...
    const handleSubmit = (editor, info) => api => {
      ...
      const attachState = {
        href: data.url.value,
        attach: data.url.meta !== undeed && data.url.meta.attach ? data.url.meta.attach : noop
      };
