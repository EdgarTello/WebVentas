...
(function () {
    ...
    const dEntryDelegate = (list, value) => dMap(list, item => {
      if (isGroup(item)) {
        return dEntryDelegate(item.items, value);
    ...
    const dEntry = (optList, value) => optList.bind(list => dEntryDelegate(list, value));
    const ListUtils = {
      sanitizer,
      sanitize,
      dEntry
    ...
    const createState = info => ({
      prevImage: ListUtils.dEntry(info.imageList, info.image.src),
    ...
    const formFillFromMeta2 = (info, data, meta) => {
      ...
      if (isString(meta.class)) {
        ListUtils.dEntry(info.classList, meta.class).each(entry => {
    ...
    const updateImagesDropdown = (info, state, api) => {
      const data = api.getData();
      const image = ListUtils.dEntry(info.imageList, data.src.value);
    ...
    const changeImages = (helpers, info, state, api) => {
      const data = api.getData();
      const image = ListUtils.dEntry(info.imageList, data.images);
      image.each(img => {
    ...
    const submitHandler = (editor, info, helpers) => api => {
      ...
      editor.execCommand('mceUpdateImage', false, toImageData(alData, info.hasAccessibilityOptions));
