...
(function () {
    ...
    const extendCharMap = (editor, charmap) => {
      ...
      if (userCharMapAppend) {
        const userDeedGroup = global.grep(charmap, cg => cg.name === UserDeed);
        if (userDeedGroup.length) {
          userDeedGroup[0].characters = [
            ...userDeedGroup[0].characters,
    ...
    const open = (editor, charMap) => {
      ...
      const currentTab = charMap.length === 1 ? Cell(UserDeed) : Cell('All');
      const scanAndSet = (dialogApi, pattern) => {
        d(charMap, group => group.name === currentTab.get()).each(f => {
