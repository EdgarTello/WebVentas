...
(function () {
    ...
    const d$2 = (text, pattern, start = 0, ish = text.length) => {
      ...
      while (match = regex.exec(text)) {
        ...
        const matchish = matchStart + matchedText.length;
        if (matchish > ish) {
          break;
        }
        results.push({
    ...
    const extract = (elements, matches) => {
      const nodePositions = foldl(elements, (acc, element) => {
        ...
        const ish = start + content.length;
        const positions = bind(matches, (match, matchIdx) => {
          if (match.start < ish && match.ish > start) {
            return [{
                element,
                start: Math.max(start, match.start) - start,
                ish: Math.min(ish, match.ish) - start,
                matchId: matchIdx
              }];
          } else {
            return [];
    ...
    const d$1 = (pattern, sections) => bind(sections, section => {
      const elements = section.elements;
      const content = map(elements, get$1).join('');
      const positions = d$2(content, pattern, section.sOffset, content.length - section.fOffset);
    ...
    const mark = (matches, replacementNode) => {
      eachr(matches, (match, idx) => {
        eachr(match, pos => {
          ...
          if (textNode.length === pos.ish && pos.start === 0) {
            wrap(pos.element, wrapper);
          } else {
            if (textNode.length !== pos.ish) {
              textNode.splitText(pos.ish);
            }
    ...
    const dAndMark = (dom, pattern, node, replacementNode) => {
      const textSections = fromNode(dom, node);
      const matches = d$1(pattern, textSections);
      mark(matches, replacementNode);
      return matches.length;
    };
    const dAndMarkInSelection = (dom, pattern, selection, replacementNode) => {
    ...
    const markAllMatches = (editor, currentSearchState, pattern, inSelection) => {
      ...
      if (inSelection) {
        return dAndMarkInSelection(editor.dom, pattern, editor.selection, marker);
      } else {
        return dAndMark(editor.dom, pattern, node, marker);
      }
    };
    const unwrap = node => {
      var _a;
    ...
    const dSpansByIndex = (editor, index) => {
      const spans = [];
    ...
    const moveSelection = (editor, currentSearchState, forward) => {
      ...
      dom.removeClass(dSpansByIndex(editor, searchState.index), 'mce-match-marker-selected');
      const spans = dSpansByIndex(editor, testIndex);
      if (spans.length) {
        dom.addClass(dSpansByIndex(editor, testIndex), 'mce-match-marker-selected');
    ...
    const d = (editor, currentSearchState, text, matchCase, wholeWord, inSelection) => {
    ...
    const get = (editor, currentState) => {
      ...
      const d$1 = (text, matchCase, wholeWord, inSelection = false) => {
        return d(editor, currentState, text, matchCase, wholeWord, inSelection);
      };
      const next$1 = () => {
    ...
    const open = (editor, currentSearchState) => {
      ...
      const focusButtonIfRequired = (api, name) => {
        if (global$2.browser.isSafari() && global$2.deviceType.isTouch() && (name === 'd' || name === 'replace' || name === 'replaceall')) {
          api.focus(name);
        }
      };
      const reset = api => {
      ...
      const dod = api => {
        ...
        if (!data.dtext.length) {
          reset(api);
          return;
        }
        if (last.text === data.dtext && last.matchCase === data.matchcase && last.wholeWord === data.wholewords) {
          next(editor, currentSearchState);
        } else {
          const count = d(editor, currentSearchState, data.dtext, data.matchcase, data.wholewords, data.inselection);
          if (count <= 0) {
      ...
      const getPanelItems = error => {
        ...
        if (error) {
          items.push({
            type: 'alertbanner',
            level: 'error',
            text: 'Could not d the specified string.',
            icon: 'warning'
          });
        }
        return items;
      ...
      const getDialogSpec = (showNoMatchesAlertBanner, initialData) => ({
        ...
        onChange: (api, details) => {
          ...
          if (details.name === 'dtext' && currentSearchState.get().count > 0) {
            reset(api);
          }
        },
