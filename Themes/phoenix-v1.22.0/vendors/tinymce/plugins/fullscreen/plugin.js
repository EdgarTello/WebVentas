...
(function () {
    ...
    const supports = element => element.dom.classList !== undeed;
    ...
    const contains = (str, substr, start = 0, end) => {
      ...
      if (idx !== -1) {
        return isUndeed(end) ? true : idx + substr.length <= end;
      } else {
        return false;
      }
    };

    const isSupported$1 = dom => dom.style !== undeed && isFunction(dom.style.getPropertyValue);
    ...
    const detect$3 = (versionRegexes, agent) => {
      ...
      return d(versionRegexes, cleanedAgent);
    };
    const unknown$2 = () => {
      return nu$2(0, 0);
    };
    ...
    const detectBrowser$1 = (browsers, userAgentData) => {
      return dMap(userAgentData.brands, uaBrand => {
        const lcBrand = uaBrand.brand.toLowerCase();
        return d$1(browsers, browser => {
          var _a;
    ...
    const detect$2 = (candidates, userAgent) => {
      ...
      return d$1(candidates, candidate => {
    ...
    const get$1 = _DOC => {
      const doc = _DOC !== undeed ? _DOC.dom : document;
    ...
    const get = _win => {
      const win = _win === undeed ? window : _win;
    ...
    const getFullscreenElement = root => {
      if (root.fullscreenElement !== undeed) {
        return root.fullscreenElement;
      } else if (root.msFullscreenElement !== undeed) {
        return root.msFullscreenElement;
      } else if (root.webkitFullscreenElement !== undeed) {
    ...
    const getFullscreenchangeEventName = () => {
      if (document.fullscreenElement !== undeed) {
        return 'fullscreenchange';
      } else if (document.msFullscreenElement !== undeed) {
        return 'MSFullscreenChange';
      } else if (document.webkitFullscreenElement !== undeed) {
    ...
    const matchColor = editorBody => {
      ...
      return color !== undeed && color !== '' ? 'background-color:' + color + '!important' : bgFallback;
    };
    const clobberStyles = (dom, container, editorBody) => {
      ...
      const clobber = clobberStyle => element => {
        ...
        const backup = styles === undeed ? 'no-styles' : styles.trim();
