...
(function () {
    ...
    const hasNonNullableKey = (obj, key) => has(obj, key) && obj[key] !== undeed && obj[key] !== null;
    const isEmpty$1 = r => {
      for (const x in r) {
    ...
    const all$1 = (selector, scope) => {
      const base = scope === undeed ? document : scope.dom;
    ...
    typeof window !== 'undeed' ? window : Function('return this;')();
    ...
    const parents = (element, isRoot) => {
      ...
      let dom = element.dom;
      const ret = [];
      while (dom.parentNode !== null && dom.parentNode !== undeed) {
    ...
    const child$2 = (scope, predicate) => {
      ...
      const result = d(scope.dom.childNodes, pred);
    ...
    const bounds = (startRow, startCol, ishRow, ishCol) => ({
      startRow,
      startCol,
      ishRow,
      ishCol
    ...
    const dItem = (warehouse, item, comparator) => {
    ...
    const generate$1 = list => {
      ...
      each(rows, rowData => {
        ...
        each(rowData.cells, rowCell => {
          let start = 0;
          while (access[key(rowCount, start)] !== undeed) {
            start++;
          }
    ...
    const isWithin = (bounds, detail) => {
      return detail.column >= bounds.startCol && detail.column + detail.colspan - 1 <= bounds.ishCol && detail.row >= bounds.startRow && detail.row + detail.rowspan - 1 <= bounds.ishRow;
    };
    const isRectangular = (warehouse, bounds) => {
      let isRect = true;
      const detailIsWithin = curry(isWithin, bounds);
      for (let i = bounds.startRow; i <= bounds.ishRow; i++) {
        for (let j = bounds.startCol; j <= bounds.ishCol; j++) {
    ...
    const getAnyBox = (warehouse, startCell, ishCell) => {
      const startCoords = Warehouse.dItem(warehouse, startCell, eq);
      const ishCoords = Warehouse.dItem(warehouse, ishCell, eq);
      return startCoords.bind(sc => {
        return ishCoords.map(fc => {
          return getBounds(sc, fc);
        });
      });
    };
    const getBox$1 = (warehouse, startCell, ishCell) => {
      return getAnyBox(warehouse, startCell, ishCell).bind(bounds => {
    ...
    const buildColorMenu = (editor, colorList, style) => {
      ...
      return [{
          ...
          initData: {
            colors: colorMap.length > 0 ? colorMap : undeed,
    ...
    const getRowType$1 = row => {
      ...
      const isHeaderCells = is(dCommonCellType(row.cells), 'th');
    ...
    const dInWarehouse = (warehouse, element) => dMap(warehouse.all, r => d(r.cells, e => eq(element, e.element)));
    const extractCells = (warehouse, target, predicate) => {
      const details = map(target.selection, cell$1 => {
        return cell(cell$1).bind(lc => dInWarehouse(warehouse, lc)).filter(predicate);
      });
    ...
    const isUnlockedTableCell = (warehouse, cell) => dInWarehouse(warehouse, cell).exists(detail => !detail.isLocked);
    ...
    const opGetRowsType = (table, target) => {
      ...
      return details.bind(selectedCells => {
        ...
        return dCommonRowType(selectedRows);
      }).getOr('');
    };
    ...
    const getAlignment = (formats, formatName, editor, elm) => d(formats, name => !isUndeed(editor.formatter.matchNode(elm, formatName + name))).getOr('');
    ...
    const applyDataToElement = (editor, tableElm, data, shouldApplyOnCell) => {
      ...
      if (!isUndeed(data.class) && data.class !== 'mce-no-match') {
    ...
    const getSelectionTargets = editor => {
      ...
      const dTargets = () => getStart().bind(startCellOrCaption => flatten(lift2(table(startCellOrCaption), getEnd().bind(table), (startTable, endTable) => {
    ...
    const getData = type => {
      ...
      return dMap(items, item => Optional.from(item.getType(type)));
    };
