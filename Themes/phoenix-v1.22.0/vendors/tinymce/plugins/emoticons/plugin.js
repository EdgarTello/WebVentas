...
(function () {
    ...
    const getUserDeedEmoji = editor => {
      const userDeedEmoticons = getAppendedEmoji(editor);
      return map(userDeedEmoticons, value => ({
        keywords: [],
        category: 'user',
        ...value
      }));
    ...
    const initDatabase = (editor, databaseUrl, databaseId) => {
      ...
      const processEmojis = emojis => {
        ...
        each(emojis, (lib, title) => {
          ...
          const current = cats[entry.category] !== undeed ? cats[entry.category] : [];
      ...
      editor.on('init', () => {
        global.load(databaseId, databaseUrl).then(emojis => {
          const userEmojis = getUserDeedEmoji(editor);
