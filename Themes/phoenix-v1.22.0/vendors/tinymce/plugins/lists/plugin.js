...
(function () {
    ...
    const fromDom$1 = node => {
      if (node === null || node === undeed) {
        throw new Error('Node cannot be null or undeed');
      }
      return { dom: node };
    };
    ...
    const is$1 = (element, selector) => {
      ...
      if (dom.nodeType !== ELEMENT) {
        ...
        const elem = dom;
        if (elem.matches !== undeed) {
          return elem.matches(selector);
        } else if (elem.msMatchesSelector !== undeed) {
          return elem.msMatchesSelector(selector);
        } else if (elem.webkitMatchesSelector !== undeed) {
          return elem.webkitMatchesSelector(selector);
        } else if (elem.mozMatchesSelector !== undeed) {
    ...
    const path = (parts, scope) => {
      let o = scope !== undeed && scope !== null ? scope : Global;
      for (let i = 0; i < parts.length && o !== undeed && o !== null; ++i) {
        o = o[parts[i]];
      }
      return o;
    };
    ...
    const inBody = element => {
      ...
      if (dom === undeed || dom === null || dom.ownerDocument === null) {
        return false;
      }
    ...
    const dSubLists = parentList => filter$1(parentList.querySelectorAll(listSelector), isListNode);
    const getSelectedSubLists = editor => {
      const parentList = getParentList(editor);
      const selectedBlocks = editor.selection.getSelectedBlocks();
      if (isParentListSelected(parentList, selectedBlocks)) {
        return dSubLists(parentList);
      } else {
    ...
    const dParentListItemsNodes = (editor, elms) => {
    ...
    const getSelectedListItems = editor => {
      const selectedBlocks = editor.selection.getSelectedBlocks();
      return filter$1(dParentListItemsNodes(editor, selectedBlocks), isListItemNode);
    };
    ...
    const getClosestListHost = (editor, elm) => {
      ...
      const parentBlock = d(parentBlocks, elm => isNotForcedRootBlock(elm) && isListHost(editor.schema, elm));
    ...
    const dLastParentListNode = (editor, elm) => {
      const parentLists = editor.dom.getParents(elm, 'ol,ul', getClosestListHost(editor, elm));
      return last(parentLists);
    };
    const getSelectedLists = editor => {
      const firstList = dLastParentListNode(editor, editor.selection.getStart());
    ...
    const getSelectedListRoots = editor => {
      ...
      return d(parentLists, p => isListInsideAnLiWithFirstAndLastNotListElement(SugarElement.fromDom(p))).fold(() => getUniqueListRoots(editor, selectedLists), l => [l]);
    };
    const getUniqueListRoots = (editor, lists) => {
      const listRoots = map(lists, list => dLastParentListNode(editor, list).getOr(list));
      return unique(listRoots);
    };

    const isCustomList = list => /\btox\-/.test(list.className);
    const inList = (parents, listName) => dUntil(parents, isListNode, isTableCellNode).exists(list => list.nodeName === listName && !isCustomList(list));
    ...
    const isSupported = dom => dom.style !== undeed && isFunction(dom.style.getPropertyValue);
    ...
    const closestSiblingEntry = (entries, start) => {
      ...
      return dUntil(reverse(entries.slice(0, start)), matches, until).orThunk(() => dUntil(entries.slice(start + 1), matches, until));
    };
    ...
    const getItemSelection = editor => {
      ...
      return lift2(d(selectedListItems, not(hasFirstChildList)), d(reverse(selectedListItems), not(hasFirstChildList)), (start, end) => ({
        start,
        end
      }));
    };
    ...
    const getEndPointNode = (editor, rng, start, root) => {
      ...
      const dBlockAncestor = node => {
        while (!editor.dom.isBlock(node) && node.parentNode && root !== node) {
          node = node.parentNode;
        }
        return node;
      };
      const dBetterContainer = (container, forward) => {
        var _a;
        const walker = new global$5(container, dBlockAncestor(container));
      ...
      if (start && isTextNode$1(container)) {
        if (isZwsp(container.textContent)) {
          container = dBetterContainer(container, false).getOr(container);
        } else {
      ...
      if (!start && isTextNode$1(container)) {
        if (isZwsp(container.textContent)) {
          container = dBetterContainer(container, true).getOr(container);
        } else {
          if (container.parentNode !== null && isInline(editor, container.parentNode)) {
            container = container.parentNode;
          }
    ...
    const dNextCaretContainer = (editor, rng, isForward, root) => {
    ...
    const backspaceDeleteFromListToListCaret = (editor, isForward) => {
      ...
      if (li) {
        ...
        const otherLi = dom.getParent(dNextCaretContainer(editor, rng, isForward, root), 'LI', root);
    ...
    const backspaceDeleteIntoListCaret = (editor, isForward) => {
      ...
      if (block && dom.isEmpty(block, undeed, { checkRootAsContent: true })) {
        const rng = normalizeRange(editor.selection.getRng());
        const otherLi = dom.getParent(dNextCaretContainer(editor, rng, isForward, root), 'LI', root);
        if (otherLi) {
          const dValidElement = element => contains$1([
            'td',
            'th',
            'caption'
          ], name(element));
          const dRoot = node => node.dom === root;
          const otherLiCell = closest$2(SugarElement.fromDom(otherLi), dValidElement, dRoot);
          const caretCell = closest$2(SugarElement.fromDom(rng.startContainer), dValidElement, dRoot);
    ...
    const register$2 = editor => {
      ...
      editor.addCommand('InsertDeitionList', (ui, detail) => {
      ...
      editor.addQueryStateHandler('InsertDeitionList', queryListCommandState(editor, 'DL'));
    };
