...
(function () {
    ...
    const dWordEnd = (characters, startIndex) => {
      let i;
    ...
    const dUrlEnd = (characters, startIndex) => {
      const endIndex = dWordEnd(characters, startIndex + 1);
      const peakedWord = characters.slice(startIndex + 1, endIndex).join(EMPTY_STRING);
      return peakedWord.substr(0, 3) === '://' ? endIndex : startIndex;
    };
    const dWordsWithIndices = (chars, sChars, characterMap, options) => {
      const words = [];
      const indices = [];
      let word = [];
      for (let i = 0; i < characterMap.length; ++i) {
        ...
        if (isWordBoundary(characterMap, i)) {
          ...
          if ((options.includeWhitespace || !WHITESPACE.test(ch)) && (options.includePunctuation || !PUNCTUATION.test(ch))) {
            ...
            if (isProtocol(str)) {
              const endOfUrl = dUrlEnd(sChars, i);
    ...
    const getWordsWithIndices = (chars, extract, options) => {
      ...
      return dWordsWithIndices(chars, extractedChars, characterMap, options);
    };
